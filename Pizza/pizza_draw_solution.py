...
def draw_solution(n, m, pizzerias_input, pizzerias_output):
    def get_cmap(n_, name='hsv'):
        return plt.cm.get_cmap(name, n_ + 1)

    cmap = get_cmap(len(pizzerias_output))
    fig, ax = plt.subplots()
    ax.plot([1, m + 1], [1, n + 1])
    for piz in pizzerias_output:
        ax.add_patch(Rectangle((pizzerias_input[piz.k_index][0] - piz.w, pizzerias_input[piz.k_index][1]),
                               piz.w + piz.e + 1, 1, color=cmap(piz.k_index)))
        ax.add_patch(Rectangle((pizzerias_input[piz.k_index][0], pizzerias_input[piz.k_index][1] - piz.s), 1,
                               piz.n + piz.s + 1, color=cmap(piz.k_index)))
    plt.show()
...
def pizza_solution(n, m, k, pizzerias_input):
    ...
    try:
        ...
        for piz in pizzerias_output:
            print(piz)
        draw_solution(n, m, pizzerias_input, pizzerias_output)
